...
class _WorkstationScreenState extends State<WorkstationScreen> {
  ...
  Future<void> _handleProcessDocument(ProcessSettings settings) async {
    ...
    final jobId =
        'job_${DateTime.now().millisecondsSinceEpoch.toRadixString(16)}';
  ...
  void _simulateOcrPipeline(
      DocumentViewModel docState, ProcessSettings settings, String jobId) {
    _simulationTimer?.cancel();
    ...
    final sampleBoxesPerPage = [
      ...
      [
        ...
        const BBoxItem(
          ...
          text:
              'This workstation provides sub-second document grounding with multi-threaded token extraction and real-time confidence scoring.',
          kind: 'paragraph',
          confidence: 0.94,
        ),
        const BBoxItem(
    ...
    _simulationTimer =
      ...
      switch (step) {
        ...
        case 7:
          _progressNotifier
              .handleWsFrame(const WsPageCompleteFrame(pageIdx: 0));
          _progressNotifier.handleWsFrame(const WsProgressFrame(
        ...
        case 8:
          timer.cancel();
          _progressNotifier.completeJob(
              message: 'Document OCR complete (5 blocks, 1 repaired)');
          break;
      }
    });
  }
