...
4. **The infrastructure is reusable.** `omniscribe_server.spec`, `scripts/build_windows.py`, `scripts/run_server.py`, and `hooks/hook-anyio.py` are kept in tree. The moment PyInstaller's analysis recognizes anyio (or a different bundler is chosen), the smoke test gate (`scripts/build_windows.py --smoke` must report `/api/health -> 200`) is the same.
