...
> **Audit (2026-08-11)**: ⚠️ Partial — All constants exist at cited locations: `_COLUMN_GAP_THRESHOLD=0.2` (`aligner.py:216`), `_SKIP_LINE_COST=1.0` (L290), `_SKIP_BOX_COST=0.4` (L291), `CLAHE_CLIP_LIMIT=2.0` (`preprocessing.py:20`), `CLAHE_TILE_GRID=8` (L21), JPEG qualities (50/80/80/85) and `MAX_SAFE_PIXELS=25_000_000` (`rasterizer.py:59–64`), `_ISO_639_MAP` spanning 55 lines (`postprocess.py:80–135`). However, calling these "magic numbers" is misleading — they are all named module-level constants with descriptive identifiers. The valid critique is externalization to config (already partially done: `PagePreprocessingOptions` exists).
...
| 2 | Route `POST /api/process` via Celery/background queue (§3.1) | `ocr.py:263` | Doesn't starve other requests | Medium | Open |
