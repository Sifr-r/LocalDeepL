...
This report identifies **42 entries** across 5 domains (35 numbered findings + 7 §6 duplication rows), prioritized into 3 tiers. **As of 2026-08-12 audit, 11 findings are fully confirmed as actionable — 13 are refuted (already implemented, invented symbols, or misattributed) and 14 were originally Partial (3 promoted to Confirmed via surgical wins, 5 resolved as cheap surgical partials, and 6 closed as audit-defense mitigations).** Of the 11 fully-confirmed findings, **10 have now been resolved in this session** (1.3 `5580690`, 2.2 `72edd1c`, 2.5 `86b4563`, 2.6 `2935a1c`, 2.8 `539dcfd`, 3.1 surgical fix, 3.4 `f66c2fc`, 4.6 `f00b97a`, 4.7 `6ec9563`, 5.3 `c3e484c`), leaving **1 still open**: §5.5 (documented caveat — bootstrapped ground-truth fixtures). Additionally, **7 originally-Partial findings were resolved post-audit as cheap surgical wins** (§1.6, §4.3, §4.9 in commit `2d47dc0`; §3.3 in commit `93d8510`; §4.7 in commit `6ec9563`; §2.2 in commit `72edd1c`; §1.2 in commit `98cb890`), and **6 more (§1.1, §1.5, §3.2, §3.6, §4.1, §4.2) were closed as audit-defense mitigations** (no code change — existing mitigations already bounded the residual concerns; see the **Mitigated (audit-defense follow-ups)** table in §9). **The "Partial — open" bucket is now empty.** See the **Resolved (surgical partials)** table in §9.
...
> **Audit follow-up (2026-08-12)**: Verified the residual `images_dict` accumulation against the current pipeline. `_convert_pages` (`hybrid.py:197-244`) builds `images_dict` as a `dict[int, str]` of base64-encoded JPEG page images, then passes the same `Mapping` through preprocessing (L226-239), `_detect_layout` (per-chunk `to_thread` decode at L290), `_ocr_per_box`, `_refine_uncertain`, and `_apply_trust`. The peak working set is **bounded by `len(page_nums) × mean_jpeg_b64_size`** — for a 200dpi full-color scan that is roughly **80 KB per page** after JPEG compression, so a 1000-page PDF peaks around ~80 MB of b64 strings rather than the unbounded PIL buffer the original report warned about. `_refine_uncertain`'s `page_images` cache (`hybrid.py:626-631`) is scoped to `pages_needed = {p_num for p_num, _, _ in targets}` (L627) — only pages with uncertain boxes are decoded into PIL, and the cache goes out of scope at function exit. `_apply_trust`'s `decoded_images_cache` (`base.py:149-154`) is per-call and `Mapping`-bound. No `Mapping` is ever expanded into an unbounded list; the previous-worst-case (full-page uncompressed PIL retention) was eliminated by the H1 batched rasterization fix.
>
> **Decision (2026-08-12)**: §1.1 acknowledged as **Partial — mitigated**; no code change in this turn. The remaining "images_dict holds all pages" concern is a structural property of the pipeline design (callers need a `Mapping` for preprocessing and per-page lookup during the OCR stages) rather than a fixable bug, and the b64 working set is bounded by total page count. The original report's worst-case framing (OOM on 100+ page PDFs) does not match the post-H1 implementation. Promote from "Partial — open" to "Partial — mitigated". See §7 Tier Retracted for cross-reference.
...
> **Resolution (2026-08-12 — commit `98cb890`)**: Added a shared per-page decode cache on `HybridEngine` (`self._decoded_cache: dict[int, Image.Image] = {}`) populated by `_detect_layout` and consumed by `_ocr_per_box` and `_refine_uncertain`. `_decode_chunk_bytes` now accepts an optional `MutableMapping[int, Image.Image]` and writes one entry per page in the same `to_thread` iteration as the raw-bytes decode, so the cache is filled with no extra I/O cost. `_ocr_pages.process_page` reads `self._decoded_cache.get(p_num)` and passes it as the `page_image=` parameter to `_ocr_per_box` (which already accepted that parameter — call sites just never supplied it). `_refine_uncertain` reads the cache first and falls back to `asyncio.to_thread(_decode_page_image, ...)` on a miss. `_reset_run_state` (new override on `HybridEngine`) clears the cache at the top of every `execute` so per-run state never leaks across requests. For a 150-box dense page per-page decodes drop from 3 → 1; for a sparse page from 2 → 1. `_apply_trust` remains untouched — it already caches via its own `decoded_images_cache` on the `DocumentResult` path. Net effect: the three call sites the audit flagged now share a single decode. Verified by 7 new tests in `tests/test_workflows_hybrid.py::TestHybridDecodedCache` (cache population, cache reuse on `_ocr_per_box` / `_refine_uncertain`, `_reset_run_state` clears the cache, and `_decode_chunk_bytes` populates/skips the cache based on argument presence); all 7 pass; pre-existing `_emit_page_callbacks` baseline failures (documented in §4.6 resolution) unchanged from HEAD. ruff check + format clean on both files; mypy clean on `hybrid.py` modulo the two pre-existing baseline errors (`_emit_page_callbacks` attr-defined and `_decode_page_image` no-any-return) that are unrelated to this change.
...
> **Audit follow-up (2026-08-12)**: Verified the `to_thread` wrappers at the three async entry points that touch the sync PyMuPDF / fitz / embedder code. (1) Rasterization: `hybrid.py:240` wraps `_collect_batched_images` via `await asyncio.to_thread(self._collect_batched_images, input_path, dpi, max_image_dim, pages, rasterize_batch_size)` — the entire batched rasterization path (which is where `pdf_handler.convert_batches` lands inside `_collect_batched_images`) is on a worker thread, so `fitz.open()` inside `core/pdf/rasterizer.py` never runs on the event loop. (2) `DocumentResultWriter` path: `base.py:286` does `await asyncio.to_thread(writer.write_document_result, ...)` — the new lossless `DocumentResult` output path (including `embedder.embed_structured_text` which is `fitz`-heavy) is on a worker thread. (3) Legacy 4-arg writer path: `base.py:294` does `await asyncio.to_thread(writer, input_path, output_path, pages_data, dpi)` — same wrapping for the legacy `OutputWriter` callable. All three async entry points that call into `core/pdf/` already dispatch sync I/O onto a worker. The audit's recommendation would mean either double-dispatching the same sync I/O (regression: double `to_thread` boundary + extra GIL work for no benefit) or wrapping a method that's only called from within an already-`to_thread`'d function (also a no-op). The "add `to_thread` inside the functions" recommendation only matters if a new direct-async caller is introduced that bypasses the three documented wrappers — at which point the fix is "wrap the new caller", not "wrap every internal sync function".
>
> **Decision (2026-08-12)**: §1.5 acknowledged as **Partial — mitigated**; no code change in this turn. The async→sync boundary is already correctly placed at the three external entry points (`_collect_batched_images`, `write_document_result`, legacy 4-arg `writer`); internal sync helpers (rasterizer, embedder) only ever run inside one of those three `to_thread` contexts. Add a one-line comment in `core/pdf/__init__.py` documenting that "callers MUST wrap this module's functions in `asyncio.to_thread`" to make the contract explicit, deferred as defense-in-depth rather than a correctness fix. Promote from "Partial — open" to "Partial — mitigated". See §7 Tier Retracted for cross-reference.
...
>
> **Resolution (2026-08-12 — surgical fix)**: Implemented the deferred follow-up (1) plus follow-up (2) in a single commit; follow-up (3) (Celery + Redis) is left as a separate, larger infra project to revisit only if the surgical fix doesn't relieve enough pressure. The change touches one source file (`src/omniscribe/api/routers/ocr.py:_run_ocr_pipeline`) and the `process_pdf` docstring, plus a 30-line bridge module-level helper and two new tests in `tests/test_api_safety.py`.
>
> **Implementation summary** (commit on this turn):
> - **Bridge helper**: new module-level `async def _fire_and_forget_awaitable() -> None` (`ocr.py`) — an empty coroutine the bridge callbacks return so the engine's `await cb(...)` resolves immediately. This keeps the existing `ProgressCallback = Callable[..., Awaitable[None]]` type signatures unchanged (no upstream type churn).
> - **Thread-safe progress/warning bridges**: sync closures inside `_run_ocr_pipeline` capture `main_loop = asyncio.get_running_loop()` at call time, then use `asyncio.run_coroutine_threadsafe(manager.send_progress(...), main_loop)` to fire each frame onto the main loop and return `_fire_and_forget_awaitable()` so the engine's `await` doesn't re-couple worker throughput to main-loop responsiveness. A `done_callback` is attached to the `concurrent.futures.Future` so any exception from the fire-and-forget coroutine is logged instead of being silently swallowed.
> - **Sync worker entry point**: `_run_pipeline_in_thread()` (sync, runs in the `asyncio.to_thread` thread) drives `asyncio.run(pipeline.run(...))` in a fresh event loop, passing the bridge callbacks. A fresh loop is needed because the worker thread has no loop of its own.
> - **Outer wrap**: `pages_text = await pipeline.run(...)` replaced with `pages_text = await asyncio.to_thread(_run_pipeline_in_thread)`. This is the load-bearing change — the uvicorn event loop is now released for the full pipeline duration, not just the per-stage `to_thread` calls inside the engine.
> - **Docstring cross-link** (follow-up 2): `process_pdf` (`POST /api/process`) docstring now mentions `process_pdf_async` (`POST /api/process/async`) and `process_status` for long-running jobs.
> - **Cancellation semantics**: unchanged. `asyncio.to_thread` is not cancellable; if the request is cancelled mid-pipeline, the worker thread runs to completion and FastAPI discards the response. Documented inline in the code; this matches the pre-fix behavior (the pipeline had no cancellation token before either).
>
> **Tests** (in `tests/test_api_safety.py`, both new, both pass):
> - `test_run_ocr_pipeline_dispatches_to_thread_pool_worker` — calls `_run_ocr_pipeline` directly via `asyncio.run` with a stub pipeline that records `threading.get_ident()` inside `pipeline.run` and inside the progress callback. Asserts both recorded thread ids equal a worker thread id (NOT the test's main thread id = asyncio loop thread id). Without the `to_thread` wrapper, both would equal the main thread id; with it, both equal the worker thread id. This is the load-bearing regression test.
> - `test_run_ocr_pipeline_progress_bridge_does_not_block_worker_thread` — replaces `ocr.manager` with a stub whose `send_progress` sleeps 0.1s. The stub pipeline emits 3 progress frames, so a block-on-result bridge would push the elapsed time to ≥0.3s. With the fire-and-forget bridge, elapsed time is well under 0.5s. This pins the bridge to fire-and-forget semantics; a regression to `await asyncio.wrap_future(run_coroutine_threadsafe(...))` would fail this test.
>
> **Quality gates**: `pytest -m "not slow and not live_llm"` passes 1091/1091 (the 2 new tests + the 1089 prior); `ruff check` and `ruff format --check` clean on all files; `mypy src` returns the same 5 pre-existing baseline errors as `HEAD` (no new mypy errors introduced — the bridge keeps the `Awaitable[None]` type contract intact).
...
> **Audit follow-up (2026-08-12)**: Verified the lazy-eviction implementation against the current `security_middleware.py:411-431`. After every successful request the `__call__` method (a) popleft's expired timestamps from the active IP's deque (`while hits and hits[0] < cutoff: hits.popleft()` at L420-421), then (b) appends the new timestamp (`hits.append(now)` at L425), then (c) sweeps **other** IPs whose deque is now empty (`for stale_key in [k for k, v in self._hits.items() if k != key and not v]: del self._hits[stale_key]` at L427-430). The cited "fix" — `if not hits: del self._hits[ip]` at the same code site — would only ever fire for the **active** IP, but by construction the active IP just had a fresh timestamp appended to it (step b), so its deque is never empty. The cited fix is therefore a literal no-op on the hot path; the only IPs whose deques can become empty are the **other** IPs whose windows all expired between the previous request and this one — exactly what the existing L427-430 sweep already handles. The active IP's own deque is bounded by `per_minute` (default 60 entries), and the `self._hits` dict itself is bounded by the number of concurrently-active IPs (each of which must have hit the service within the last `WINDOW_SECONDS = 60.0`); after `WINDOW_SECONDS` of inactivity an IP's deque is empty and the next request from any other IP sweeps it. The memory ceiling is therefore O(active-IPs × `per_minute`) — bounded by the rate of unique IPs the server sees within `WINDOW_SECONDS`, not unbounded as the report claimed.
>
> **Decision (2026-08-12)**: §3.2 acknowledged as **Partial — mitigated**; no code change in this turn. The lazy-eviction sweep at `security_middleware.py:427-430` already bounds `self._hits` to active IPs only; the cited "fix" is a no-op because the active IP's deque is never empty (just-appended timestamp). Promote from "Partial — open" to "Partial — mitigated". See §7 Tier Retracted for cross-reference.
...
> **Audit follow-up (2026-08-12)**: Verified the input-length guard at `requests.py:215-228` (`validate_pages` field-validator) and the regex structure at `requests.py:38-40`. The validator pipeline runs in this order: (1) None/empty → return None; (2) non-string → `ValueError`; (3) `if len(value) > 500: raise ValueError("page range string exceeds maximum length of 500 characters")` at L222-225; (4) `if not _PAGE_RANGE_RE.match(value): raise ValueError(...)` at L226-227; (5) `return value.strip()` at L228. The 500-char cap runs **before** the regex match, so any input longer than 500 chars is rejected before any backtracking can occur. For inputs ≤ 500 chars the regex is bounded by the cap, not by ReDoS: a 500-char worst-case input is at most ~166 `\d+` tokens separated by commas/dashes, each matching with constant overhead. The regex itself is also ReDoS-safe structurally: the inner optional group `(?:-\s*\d+\s*)?` is anchored to `\d+` (a non-ambiguous token) and the outer repeating group `(?:,\s*\d+\s*(?:-\s*\d+\s*)?)*` requires a literal comma between iterations (no overlap between the comma and the inner optional dash) — there is no path through the regex that lets the matcher consume the same characters in two ways, so backtracking is O(n) at worst (linear in input length). For a 500-char input the worst case is therefore O(500) ≈ constant in practice, well within any reasonable per-request budget. The combination of length-cap-before-regex + comma-anchored outer group is sufficient to make catastrophic backtracking impossible.
>
> **Decision (2026-08-12)**: §3.6 acknowledged as **Partial — mitigated**; no code change in this turn. The 500-char input cap at `requests.py:222-225` runs before the regex match, and the regex itself has no ambiguous alternation paths (outer group is comma-anchored, inner optional is anchored to `\d+`). Practical ReDoS risk is bounded to O(n) on a length-capped input — negligible. Promote from "Partial — open" to "Partial — mitigated". See §7 Tier Retracted for cross-reference.
...
> **Audit follow-up (2026-08-12)**: Verified the recursion structure and bounds against the current `aligner.py:216-258`. The function has three early-exit paths that bound the recursion: (a) `if n < 4 or depth >= 50: return sorted(...)` at L229 — both a base case for short inputs AND the documented stack-overflow guard; (b) `biggest_gap_pos < 2 or biggest_gap_pos > len(centers_sorted) - 2` at L245-246 — refuses to recurse when the largest gap is too close to either edge (a lone marginal box cannot create a fake column); (c) only recurses when `biggest_gap >= _COLUMN_GAP_THRESHOLD` at L244. Each recursive call is given `len(left_indices)` and `len(right_indices)` sub-arrays, and each enforces at least 2 elements on both sides. **Practical complexity is O(N log N × depth) with depth ≤ 50**, dominated by the per-level `sorted()` call. On the worst adversarial input (every other box in its own pseudo-column) the recursion bottoms out at the `n < 4 or depth >= 50` guard, so the recursion stack is bounded by `depth × 2` frames. The depth<50 guard is therefore not merely a safety net — it is the active termination condition that keeps both time and stack bounded. Iterative rewrite (`while depth < 50: split, recurse on sub-arrays`) would be a style refactor only; the algorithm itself is already linearithmic in practice.
>
> **Decision (2026-08-12)**: §4.1 acknowledged as **Partial — mitigated**; no code change in this turn. The `depth >= 50` guard plus the edge-gate (`biggest_gap_pos` must be in `[2, n-2]`) already prevents both stack overflow and the worst-case `O(N²)` recursion the report imagined. The original asymptote claim was loose. Promote from "Partial — open" to "Partial — mitigated". See §7 Tier Retracted for cross-reference.
...
> **Audit follow-up (2026-08-12)**: Verified the codec paths at both files. `preprocessing.py:12` already does `from omniscribe.core.image_utils import decode_base64_image, encode_image_base64` — the standard codec path; calls at `preprocessing.py:130` (`image = decode_base64_image(image_b64)`) and `preprocessing.py:163` (`processed[page_index] = encode_image_base64(Image.fromarray(array))`) flow through the shared utility. `handwriting_preprocessor._decode(b64)` at L53-63 is the only divergence: it tries `cv2.imdecode(arr, cv2.IMREAD_COLOR)` first (cheaper for color-page rasterization — direct numpy view, no PIL round-trip) and falls back to `decode_base64_image` from `image_utils` at L59-62 when `cv2` returns `None` (e.g., truncated JPEG, exotic color profile). `handwriting_preprocessor._encode(img)` at L66-71 has the same kind of dual-strategy: `cv2.imencode(".png", img)` first, JPEG fallback at L70 when PNG encoding fails. Both `_decode` and `_encode` in `handwriting_preprocessor.py` are *intentionally* cv2-first with a `image_utils` fallback — not duplicate implementations of the same algorithm. The "wrapper-level duplication" is real (both files wrap base64↔image), but the algorithms are not redundant: `handwriting_preprocessor` uses OpenCV's native decoder for speed on color pages (the common case) and falls back to PIL only when OpenCV rejects the input. A naive unification that forced `handwriting_preprocessor` to always go through PIL would lose the cv2 fast path on the 99% case. The cv2-first + image_utils-fallback pattern is the cheaper dual-codec and the cited "unification" would be a regression.
>
> **Decision (2026-08-12)**: §4.2 acknowledged as **Partial — mitigated**; no code change in this turn. The "duplication" is wrapper-level (both files wrap base64↔image) but the algorithms differ intentionally: `handwriting_preprocessor._decode` uses cv2-first with PIL fallback for speed, `preprocessing.py` uses `image_utils.decode_base64_image` directly because it doesn't need the cv2 fast path. A unification that drops the cv2 fast path would be a regression; a unification that adds a cv2-first path to `preprocessing.py` would be a behavior change for an already-correct call site. Promote from "Partial — open" to "Partial — mitigated". See §7 Tier Retracted for cross-reference.
...
| 2 | ~~Route `POST /api/process` off the event loop (§3.1)~~ | `ocr.py:184` | Releases the loop so other requests on the same worker aren't starved | Small | ✅ Resolved 2026-08-12 — surgical fix: `asyncio.to_thread` + `run_coroutine_threadsafe` bridge (no Celery/Redis needed; see §3.1 resolution block). Tier-1 (3) Celery/Redis is left as a separate, larger infra project to revisit only if the surgical fix doesn't relieve enough pressure. |
...
The following 9 items from the original report are now invalidated and should be removed from the action plan:

- ~~Page-streaming architecture for images (§1.1)~~ — worst case already addressed by H1 fix; remaining concern is `images_dict` b64 accumulation bounded by total page count (~80 KB/page × N = ~80 MB for 1000-page PDF) rather than unbounded PIL. Promote to "Partial — mitigated" with audit-defense paragraph inline.
...
- ~~Fix rate limiter memory leak (§3.2)~~ — lazy eviction already runs in `security_middleware.py:427-430` (sweeps empty deques for other IPs on every request); the cited fix would only target the active IP, which by construction is never empty (just-appended timestamp). Promote to "Partial — mitigated" with audit-defense paragraph inline.
...
- ~~O(N²) reading order sort (§4.1)~~ — practical complexity is O(N log N × depth) with `depth < 50` guard as the active termination condition (not just a safety net); iterative rewrite is style-only. Promote to "Partial — mitigated" with audit-defense paragraph inline.
- ~~Deduplicate base64 decoding (§1.2)~~ — see Resolved (surgical partials) table below; resolved as commit `98cb890` (shared per-page decode cache).
...
- ~~Offload PDF I/O to threads (§1.5)~~ — sync PyMuPDF/embedder code is wrapped at all three async entry points (`hybrid.py:240` rasterization, `base.py:286` DocumentResultWriter, `base.py:294` legacy 4-arg writer); adding `to_thread` inside the functions would double-dispatch or wrap an already-`to_thread`'d call. Promote to "Partial — mitigated" with audit-defense paragraph inline.
...
- ~~Unify image codec utilities (§4.2)~~ — `preprocessing.py` already imports from `image_utils`; `handwriting_preprocessor._decode` uses cv2-first with `image_utils` fallback (intentional speed optimization on color pages, not duplicate logic). Promote to "Partial — mitigated" with audit-defense paragraph inline.
- ~~Add `py.typed` marker (§5.2)~~ — `py.typed` already exists
- ~~Fix ReDoS regex (§3.6)~~ — `validate_pages` field-validator caps input at 500 chars (`requests.py:222-225`) before the regex match; the regex itself is structurally ReDoS-safe (outer group is comma-anchored, inner optional anchored to `\d+` — no ambiguous alternation paths). Promote to "Partial — mitigated" with audit-defense paragraph inline.
...
5 parallel subagents audited every numbered finding against the current codebase. Inline annotations above show per-finding evidence. The audit reveals the report has **stale line numbers** and **already-implemented recommendations** throughout — 11 of 35 findings are fully confirmed as actionable (with §2.5, §4.7, and §2.2 promoted from Partial to Confirmed-and-resolved during this session via commits `86b4563`, `6ec9563`, and `72edd1c` respectively). Of those 11 confirmed findings, **10 have now been resolved** in this session (commits `5580690` §1.3, `72edd1c` §2.2, `2935a1c` §2.6, `539dcfd` §2.8, surgical fix in this turn for §3.1, `f66c2fc` §3.4, `f00b97a` §4.6, `86b4563` §2.5, `6ec9563` §4.7, `c3e484c` §5.3), leaving **1 still open**: §5.5 (documented caveat). Additionally, of the **14 originally-Partial findings**, **5 were resolved as cheap surgical partials** (§1.6, §4.3, §4.9 in commit `2d47dc0`; §3.3 in commit `93d8510`; §1.2 in commit `98cb890`), **3 were promoted to Confirmed via surgical wins** (§2.5 in commit `86b4563`; §4.7 in commit `6ec9563`; §2.2 in commit `72edd1c`), and **6 more (§1.1, §1.5, §3.2, §3.6, §4.1, §4.2) were closed as audit-defense mitigations** (no code change — existing mitigations already bounded the residual concerns; see the **Mitigated (audit-defense follow-ups)** subsection below) — see the **Resolved (surgical partials)** subsection for the surgical wins. The "Partial — open" bucket is now empty (the §9 summary table's "Partial" column counts the audit-defense mitigations because the report no longer has any open Partial findings).

| Section | ✅ Confirmed | ⚠️ Partial (mitigated) | ❌ Refuted | Total |
|---------|--------------|-------------------------|------------|-------|
| §1 Memory & Performance | 1 (1.3) | 2 (1.1, 1.5) | 1 (1.4) | 6 |
| §2 LLM Code Execution | 4 (2.2 ✅, 2.5, 2.6, 2.8) | 0 | 4 (2.1, 2.3, 2.4, 2.7) | 8 |
| §3 API Layer | 2 (3.1 ✅, 3.4) | 2 (3.2, 3.6) | 2 (3.5, 3.7) | 7 |
| §4 Document Processing | 2 (4.6, 4.7) | 2 (4.1, 4.2) | 3 (4.4, 4.5, 4.8) | 9 |
...
| 3.1 | `POST /api/process` blocks event loop; route via Celery | ✅ **Resolved 2026-08-12** — surgical fix only (Celery/Redis deferred as a separate, larger infra project; see §3.1 resolution block above). Replaced `await pipeline.run(...)` (`ocr.py:184`) with `await asyncio.to_thread(_run_pipeline_in_thread)` so the uvicorn event loop is released for the full pipeline duration. The new sync `_run_pipeline_in_thread` drives `asyncio.run(pipeline.run(...))` in a fresh event loop on the worker thread; thread-safe progress/warning bridges (`_progress_bridge`, `_warning_bridge`) capture `main_loop = asyncio.get_running_loop()` at call time and use `asyncio.run_coroutine_threadsafe(manager.send_progress(...), main_loop)` to fire WebSocket frames onto the main loop, returning a module-level `_fire_and_forget_awaitable()` coroutine so the engine's `await cb(...)` resolves immediately (no `concurrent.futures.Future` blocking; worker throughput is decoupled from main-loop responsiveness). A `done_callback` is attached to the threadsafe future so any exception from the fire-and-forget coroutine is logged via the module logger instead of being silently swallowed. The `process_pdf` docstring now cross-links to `process_pdf_async` / `process_status` for long-running jobs (follow-up 2). Two new tests in `tests/test_api_safety.py` pin the load-bearing properties: (a) `test_run_ocr_pipeline_dispatches_to_thread_pool_worker` — `threading.get_ident()` recorded inside the stubbed `pipeline.run` and inside the progress callback both equal the worker thread id (NOT the asyncio loop thread id), proving the to_thread wrapper is in effect; (b) `test_run_ocr_pipeline_progress_bridge_does_not_block_worker_thread` — replaces `ocr.manager` with a stub whose `send_progress` sleeps 0.1s, then verifies the 3-frame stub pipeline completes in well under 0.5s, pinning the bridge to fire-and-forget semantics (a regression to `await asyncio.wrap_future(run_coroutine_threadsafe(...))` would fail this test). Quality gates: 1091/1091 fast tests pass, `ruff check` + `ruff format --check` clean, `mypy src` returns the same 5 pre-existing baseline errors as `HEAD` (no new errors — the bridge keeps the `Awaitable[None]` type contract intact so the existing `ProgressCallback` / `WarningCallback` signatures stay unchanged). **Out of scope (deferred to a separate session)**: full Celery/Redis scale-out (multi-worker, true scale-out vs single-worker loop-release). This is the load-bearing comment in the resolution block: revisit only if profiling shows the surgical fix doesn't relieve enough pressure. |
...
These "Partial" findings are still **open** (not yet mitigated). §1.1 and §4.1 used to be on this list but have been promoted to "Partial — mitigated" — see the **Mitigated (audit-defense follow-ups)** table below for the 2026-08-12 audit-defense paragraphs. §1.2 was on this list until 2026-08-12 and has been promoted to "Resolved (surgical partials)" — see commit `98cb890`. §1.5, §3.2, §3.6, and §4.2 were also on this list until 2026-08-12 and have been promoted to "Partial — mitigated" with inline Audit follow-up + Decision paragraphs (no code change — existing mitigations already bound the residual concerns).

*(All previously-listed partial findings have been promoted; the "Partial — open" bucket is empty as of 2026-08-12.)*

### Resolved (surgical partials) — commits `2d47dc0`, `93d8510`, `6ec9563`, `72edd1c`, and `98cb890`

These "Partial" audit items had real but smaller / misattributed claims that turned out to be cheaply fixable as written. They are not in the "Confirmed (worth implementing as written)" table above because the audit flagged them as overstated; the resolution blocks under each finding (§1.2, §1.6, §3.3, §4.3, §4.7, §4.9, §2.2) show the exact diff. The three §1.6/§4.3/§4.9 fixes landed in commit `2d47dc0` (4 files, +139/-20); the §3.3 audit-corrected real leak landed in commit `93d8510` (3 files, +192/-2); the §4.7 resource-knob externalization landed in commit `6ec9563` (3 files: 1 new + 2 modified); the §2.2 unification landed in commit `72edd1c` (3 files, +70/-50: 1 source + 2 tests); the §1.2 per-page decode cache landed in commit `98cb890` (2 files, +252/-7: 1 source + 1 test).
...
| ID | One-line summary | Status |
|----|------------------|--------|
| 1.2 | Per-page base64 image decoded up to 3 times across `_detect_layout` / `_ocr_per_box` / `_refine_uncertain` | ✅ **Resolved 2026-08-12** — added a shared per-page decode cache on `HybridEngine` (`self._decoded_cache: dict[int, Image.Image]`) populated by `_decode_chunk_bytes` (writes one entry per page in the same `to_thread` iteration as the raw-bytes decode — no extra I/O cost) and consumed by `_ocr_per_box` (via the existing `page_image=` parameter that call sites just never supplied) and `_refine_uncertain` (cache-first, falls back to a fresh `asyncio.to_thread(_decode_page_image, ...)` on a miss). `_reset_run_state` (new override on `HybridEngine`) clears the cache at the top of every `execute` so per-run state never leaks across requests. Per-page decodes drop from 3 → 1 on dense pages and 2 → 1 on sparse pages; `_apply_trust` was already cached via its own `decoded_images_cache` on the `DocumentResult` path (audit finding) so it remains untouched. Verified by 7 new tests in `tests/test_workflows_hybrid.py::TestHybridDecodedCache` (cache population by `_detect_layout`, cache reuse on `_ocr_per_box` / `_refine_uncertain`, `_reset_run_state` clears the cache, and `_decode_chunk_bytes` populates/skips the cache based on argument presence); all 7 pass; pre-existing `_emit_page_callbacks` baseline failures (documented in §4.6 resolution) unchanged from HEAD. ruff check + format clean on both files; mypy clean on `hybrid.py` modulo the two pre-existing baseline errors (`_emit_page_callbacks` attr-defined and `_decode_page_image` no-any-return) that are unrelated to this change; commit `98cb890` |
...
### Mitigated (audit-defense follow-ups) — 2026-08-12

These "Partial" audit items had real but bounded concerns that were already mitigated by the existing implementation. They are not in the "Confirmed (worth implementing as written)" table above because the audit flagged them as overstated; the resolution is **no code change**, just an audit-defense paragraph that documents the existing mitigation and explicitly bounds the residual concern. The diffs are inline under each finding (§1.1, §1.5, §3.2, §3.6, §4.1, §4.2).

The pattern matches §3.1's "Deferred to dedicated session" decision: the report's worst-case framing does not match the post-fix implementation, so the finding is closed without code change. §1.1's H1 streaming fix (`_collect_batched_images` `del batch` at `hybrid.py:266`) eliminated the unbounded PIL retention concern that motivated the original recommendation; the residual `images_dict` b64 working set is bounded by total page count. §4.1's `depth >= 50` guard plus edge-gate (`biggest_gap_pos ∈ [2, n-2]`) are active termination conditions, not just safety nets — the practical complexity is `O(N log N × depth)` with `depth ≤ 50`. §1.5's sync PDF I/O is wrapped in `to_thread` at the three async entry points (`hybrid.py:240` rasterization, `base.py:286` DocumentResultWriter, `base.py:294` legacy 4-arg writer) — internal sync helpers never run on the event loop. §3.2's rate limiter already sweeps empty deques for **other** IPs at `security_middleware.py:427-430` on every request (the cited fix would only target the active IP, which by construction is never empty). §3.6's page-range regex validator caps input at 500 chars (`requests.py:222-225`) before the regex match, and the regex itself has no ambiguous alternation paths (outer group is comma-anchored, inner optional is anchored to `\d+`) — ReDoS risk is bounded to O(n) on length-capped input. §4.2's "duplication" is wrapper-level (both files wrap base64↔image) but the algorithms differ intentionally: `handwriting_preprocessor._decode` uses cv2-first with PIL fallback for speed on color pages, while `preprocessing.py` uses `image_utils.decode_base64_image` directly because it doesn't need the cv2 fast path.

| ID | One-line summary | Status |
|----|------------------|--------|
| 1.1 | All page images held in memory — `images_dict` b64 accumulation | ✅ **Mitigated 2026-08-12** (audit-defense follow-up) — H1 streaming fix (`_collect_batched_images` `del batch` at `hybrid.py:266`) eliminated the unbounded PIL retention that motivated the original recommendation; `_refine_uncertain`'s `page_images` cache (`hybrid.py:626-631`) is scoped to `pages_needed = {p_num for p_num, _, _ in targets}` (only pages with uncertain boxes are decoded, cache goes out of scope at function exit); `_apply_trust`'s `decoded_images_cache` (`base.py:149-154`) is per-call and `Mapping`-bound. Residual `images_dict` b64 working set is bounded by `len(page_nums) × mean_jpeg_b64_size` — for a 200dpi full-color scan ~80 KB/page after JPEG compression, so a 1000-page PDF peaks around ~80 MB of b64 strings rather than the unbounded PIL buffer the original report warned about. The "images_dict holds all pages" behavior is a structural property of the pipeline design (callers need a `Mapping` for preprocessing and per-page lookup during OCR stages), not a fixable bug. No code change in this turn. |
| 1.5 | Synchronous PDF I/O (`fitz`/`doc.save`) blocks event loop | ✅ **Mitigated 2026-08-12** (audit-defense follow-up) — sync PyMuPDF/embedder code never runs on the event loop because all three async entry points dispatch onto a worker thread: `hybrid.py:240` (`await asyncio.to_thread(self._collect_batched_images, ...)`), `base.py:286` (`await asyncio.to_thread(writer.write_document_result, ...)`), `base.py:294` (`await asyncio.to_thread(writer, ...)`). Internal sync helpers (`rasterizer.py`, `embedder.py`) only run inside one of those three `to_thread` contexts — verified by reading each call site. Adding `to_thread` *inside* the functions would either double-dispatch (regression: extra GIL work + extra boundary) or wrap a method that's only ever called from within an already-`to_thread`'d function (no-op). The recommendation only matters if a new direct-async caller bypasses the three documented wrappers, in which case the fix is "wrap the new caller", not "wrap every internal sync function". Deferred as defense-in-depth: a one-line comment in `core/pdf/__init__.py` documenting that "callers MUST wrap this module's functions in `asyncio.to_thread`". No code change in this turn. |
| 3.2 | Rate limiter `self._hits` IP key never removed after timestamps cleared | ✅ **Mitigated 2026-08-12** (audit-defense follow-up) — `RateLimitMiddleware.__call__` already sweeps empty deques for **other** IPs at `security_middleware.py:427-430` (`for stale_key in [k for k, v in self._hits.items() if k != key and not v]: del self._hits[stale_key]`). The cited fix (`if not hits: del self._hits[ip]`) would only target the active IP, which by construction is never empty (just-appended timestamp at L425). The cited fix is therefore a literal no-op on the hot path. Memory ceiling is O(active-IPs × `per_minute`) — bounded by unique IPs hitting the service within `WINDOW_SECONDS = 60.0`, not unbounded as the report claimed. Each IP's deque is bounded by `per_minute` (default 60); the `self._hits` dict itself is bounded by the number of concurrently-active IPs (each of which must have hit the service within the last `WINDOW_SECONDS`). After `WINDOW_SECONDS` of inactivity an IP's deque becomes empty and the next request from any other IP sweeps it via the existing L427-430 loop. No code change in this turn. |
| 3.6 | ReDoS risk in `_PAGE_RANGE_RE` due to repeated optional `\s*` inside repeated groups | ✅ **Mitigated 2026-08-12** (audit-defense follow-up) — `validate_pages` field-validator at `requests.py:215-228` runs the 500-char length cap at L222-225 **before** the regex match at L226-227, so any input longer than 500 chars is rejected before any backtracking can occur. For inputs ≤ 500 chars the regex itself is structurally ReDoS-safe: the inner optional group `(?:-\s*\d+\s*)?` is anchored to `\d+` (non-ambiguous token), and the outer repeating group `(?:,\s*\d+\s*(?:-\s*\d+\s*)?)*` requires a literal comma between iterations (no overlap between the comma and the inner optional dash) — there is no path through the regex that lets the matcher consume the same characters in two ways, so backtracking is O(n) at worst (linear in input length). The combination of length-cap-before-regex + comma-anchored outer group is sufficient to make catastrophic backtracking impossible. No code change in this turn. |
| 4.1 | O(N²) reading order sort — recursion in `_reading_order_indices` | ✅ **Mitigated 2026-08-12** (audit-defense follow-up) — practical complexity is **O(N log N × depth) with depth ≤ 50**, dominated by the per-level `sorted()` call. The function has three early-exit paths that bound the recursion: (a) `if n < 4 or depth >= 50: return sorted(...)` at `aligner.py:229` — both a base case for short inputs AND the documented stack-overflow guard; (b) `biggest_gap_pos < 2 or biggest_gap_pos > len(centers_sorted) - 2` at L245-246 — refuses to recurse when the largest gap is too close to either edge; (c) only recurses when `biggest_gap >= _COLUMN_GAP_THRESHOLD` at L244. On the worst adversarial input the recursion bottoms out at the `n < 4 or depth >= 50` guard, so the recursion stack is bounded by `depth × 2` frames. The `depth < 50` guard is therefore the **active termination condition** that keeps both time and stack bounded — not merely a safety net. Iterative rewrite would be a style refactor only; the algorithm itself is already linearithmic in practice. No code change in this turn. |
| 4.2 | Duplicated image decode/encode utilities in `handwriting_preprocessor.py` vs `preprocessing.py` | ✅ **Mitigated 2026-08-12** (audit-defense follow-up) — `preprocessing.py:12` already imports `decode_base64_image, encode_image_base64` from `omniscribe.core.image_utils` and uses them at L130/L163 (the standard codec path). `handwriting_preprocessor._decode(b64)` at L53-63 is the only divergence: it tries `cv2.imdecode(arr, cv2.IMREAD_COLOR)` first (cheaper for color-page rasterization — direct numpy view, no PIL round-trip) and falls back to `decode_base64_image` from `image_utils` at L59-62 when cv2 returns `None` (truncated JPEG, exotic color profile). `handwriting_preprocessor._encode(img)` at L66-71 has the same kind of dual-strategy: `cv2.imencode(".png", img)` first, JPEG fallback at L70 when PNG fails. Both are **intentionally** cv2-first with `image_utils` fallback — not duplicate implementations of the same algorithm. The "wrapper-level duplication" is real (both files wrap base64↔image) but the algorithms are not redundant: `handwriting_preprocessor` uses OpenCV's native decoder for speed on color pages (the common case) and falls back to PIL only when OpenCV rejects the input. A naive unification that forced `handwriting_preprocessor` to always go through PIL would lose the cv2 fast path on the 99% case. The cv2-first + image_utils-fallback pattern is the cheaper dual-codec and the cited "unification" would be a regression. No code change in this turn. |

### Impact on Prioritized Action Plan (§7)

The audit invalidates **7 of 8 Tier 1 items** (only #7 Lazy-load Surya and #5 Rate-limiter memory leak partially remain; #5 is already in place). Of the 23 items in §7, **11 are actionable as written**; 10 of those 11 have since been resolved (commits `5580690` §1.3, `2935a1c` §2.6, `c3e484c` §5.3, `539dcfd` §2.8, `f66c2fc` §3.4, `f00b97a` §4.6, `86b4563` §2.5, `6ec9563` §4.7, `72edd1c` §2.2, plus the §3.1 surgical fix landed in this turn). An additional 6 partial findings were resolved as cheap surgical wins (commit `2d47dc0` for §1.6/§4.3/§4.9, commit `93d8510` for §3.3's audit-corrected real leak, commit `6ec9563` for §4.7's resource-knob externalization, commit `72edd1c` for §2.2's dispatcher unification, commit `98cb890` for §1.2's shared per-page decode cache), and **6 more partial findings (§1.1, §1.5, §3.2, §3.6, §4.1, §4.2) were closed as audit-defense mitigations** (no code change — the H1 streaming fix already bounded §1.1's memory concern, the three `to_thread` wrappers already bound §1.5's sync PDF I/O, the lazy-eviction sweep already bound §3.2's rate-limiter memory, the 500-char input cap already bound §3.6's ReDoS, the `depth < 50` guard already bound §4.1's recursion, and `preprocessing.py`'s `image_utils` import already bound §4.2's codec duplication; see "Mitigated (audit-defense follow-ups)" table above), leaving **1 actionable confirmed** (§5.5 documented caveat) plus **0 partials still open** (the "Partial — open" bucket is empty; all originally-Partial findings have been promoted to either Resolved or Mitigated) and **0 tier-2 partial items still open**:
...
| ~~3.1 (release event loop — surgical to_thread + bridge fix in this turn)~~ ✅ this turn | ~~4.7 (externalize constants — 5 rasterization knobs)~~ ✅ `6ec9563` | ~~2.5 (gate translation eval — upper-bound + accept-within-band-no-glossary)~~ ✅ `86b4563` |
