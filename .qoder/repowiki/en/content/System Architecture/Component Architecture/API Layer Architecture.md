...
## Update Summary
**Changes Made**
- Added documentation for new router modules: glossary_imports.py, providers.py, and state.py
- Updated config.py section to reflect significant expansion from 169 to 455 lines
- Enhanced router organization section with new feature modules
- Updated dependency analysis to include new router dependencies
- Revised project structure to reflect current file organization under src/omniscribe
...
## Introduction
This document describes the API layer architecture of LocalDeepL (now Omniscribe), focusing on the FastAPI router structure, request/response handling patterns, and endpoint organization. It explains how REST endpoints are separated from WebSocket handlers, how Pydantic schemas are used for validation, and how errors and responses are formatted. It also covers authentication middleware integration, rate limiting implementation, security considerations, and the API versioning strategy with backward compatibility approaches.

## Project Structure
The API layer is organized under src/omniscribe/api:
- routers: Feature-based route modules including translation, extraction, artifacts, config, jobs, state, ocr, websocket, glossary_imports, and providers.
...
B --> E["REST Endpoints<br/>translation.py, extraction.py, artifacts.py,<br/>config.py, jobs.py, state.py, ocr.py,<br/>glossary_imports.py, providers.py"]
...
**Updated** Added new router modules for glossary imports, provider management, and enhanced state management capabilities.
...
- config.py: Configuration endpoints (significantly expanded).
- jobs.py: Job lifecycle endpoints.
- state.py: State inspection and management endpoints.
- ocr.py: OCR pipeline endpoints.
- glossary_imports.py: Glossary import and management endpoints.
- providers.py: Provider configuration and management endpoints.
...
**Updated** Added three new router modules: glossary_imports.py for glossary management, providers.py for provider configuration, and state.py for enhanced state management. The config.py module has been significantly expanded from 169 to 455 lines, indicating substantial growth in configuration capabilities.
...
GlossaryImportsRouter --> Schemas : "uses"
ProvidersRouter --> Schemas : "uses"
```
...
**Updated** Added new router modules (glossary_imports.py, providers.py, state.py) that extend the dependency graph with additional feature-specific dependencies.
...
Routers --> NewRouters["New Routers:<br/>glossary_imports.py,<br/>providers.py,<br/>state.py"]
```
...
- Monitor the expanded config.py module (455 lines) for potential performance implications of increased configuration complexity.

**Updated** Added consideration for monitoring the significantly expanded config.py module which now contains 455 lines compared to the previous 169 lines.
...
- New router issues: For glossary_imports.py, providers.py, and state.py endpoints, check specific error handling patterns and validation requirements.

**Updated** Added troubleshooting guidance for the new router modules (glossary_imports.py, providers.py, state.py).
