...
def test_no_redundant_asyncio_marks_in_active_test_files() -> None:
    ...
    assert not hits, (
        ...
        "_ASYNCIO_MARK_EXCEPTIONS in tests/test_audit_medium_d4.py with "
...
def test_synthetic_pdf_fixture_is_session_scoped() -> None:
    ...
    """
    from tests import test_chunked_runner
...
def test_docuverse_shim_no_longer_collectable() -> None:
    ...
    skips it. The content moved to a sibling ``.md`` file for
    historical reference.
    """
    assert not (TESTS_DIR / "test_docuverse_upgrade.py").exists(), (
        "test_docuverse_upgrade.py was renamed; see the test_audit_medium_d4 "
        "F4.14 fix for the new path."
    )
    md_path = TESTS_DIR / "docuverse_split_history.md"
    assert md_path.exists(), (
        "The historical context should be preserved at tests/docuverse_split_history.md"
    )
...
def test_health_endpoints_drops_module_asyncio_pytestmark() -> None:
    ...
    """
    text = (TESTS_DIR / "test_health_endpoints.py").read_text(encoding="utf-8")
...
def test_workflows_callback_decoupling_ids_use_relative_path() -> None:
    ...
    """
    text = (TESTS_DIR / "test_workflows_callback_decoupling.py").read_text(
        encoding="utf-8"
    )
...
@pytest.mark.parametrize(
    "module",
    [
        "tests/test_pdf.py",  # F4.7
        "tests/test_text_layer_recall.py",  # F4.8
        "tests/test_phase5_env_and_spellcheck.py",  # F4.13
    ],
)
...
def test_runtime_settings_filesystem_test_is_platform_independent() -> None:
    ...
    """
    from tests import test_runtime_settings
