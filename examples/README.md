...
| --- | --- | --- | --- | --- |
| `digital.pdf` | Public-domain English text rendered as a digital PDF (embedded text layer, machine-generated). | Public domain | Initial sample corpus | `tests/api/test_integration.py` (Surya detection + DP alignment regression); `scripts/visualize.py boxes|compare`, `scripts/inspect_doc.py pdf` |
| `hybrid.pdf` | Scanned page of a public-domain document with mixed embedded text + raster. | Public domain | Initial sample corpus | `tests/api/test_integration.py` (joined-paragraph regression, `tests/api/test_integration.py:346`); `scripts/visualize.py align|boxes|compare`, `scripts/inspect_doc.py pdf` |
| `handwritten.pdf` | Synthetic handwriting page used to exercise the handwriting preprocessor. | CC0 / synthetic | Initial sample corpus | `tests/api/test_integration.py`; `scripts/visualize.py boxes|compare`, `scripts/inspect_doc.py pdf` |
| `dense.pdf` | A dense, multi-column public-domain document that pushes the box count over `dense_threshold` (forces per-box OCR instead of full-page). | Public domain | Initial sample corpus | `tests/core/test_aligner.py`, `tests/fixtures/ground_truth_dense.json`; `e2e/test_ui.py` (Playwright smoke); `scripts/confidence_eval.py`, `scripts/measure_recall_delta.py`, `scripts/build_fixture.py` |
