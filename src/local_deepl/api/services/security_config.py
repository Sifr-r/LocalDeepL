...
* ``LOCAL_DEEPL_AUTH_TOKEN`` — global fallback applied to every route.
* ``LOCAL_DEEPL_OCR_AUTH_TOKEN`` — overrides the global token for OCR
  routes (``/api/process``, ``/api/models/ocr``, ``/api/config/ocr``).
  When set, the global token does **not** unlock those routes.
* ``LOCAL_DEEPL_TRANSLATION_AUTH_TOKEN`` — overrides the global token
...
class SecuritySettings:
    ...
    def from_env() -> SecuritySettings:
        ...
        Recognised variables:
          * ``LOCAL_DEEPL_AUTH_TOKEN`` — global fallback applied to every
            route. Setting it is the simplest "lock the door" knob.
          * ``LOCAL_DEEPL_OCR_AUTH_TOKEN`` — overrides the global token
            for OCR routes. When set, the global token does not unlock
            OCR routes.
          * ``LOCAL_DEEPL_TRANSLATION_AUTH_TOKEN`` — same as above for
            translation routes.
        ...
        token = _validate_auth_token(
            "LOCAL_DEEPL_AUTH_TOKEN", _env_str("LOCAL_DEEPL_AUTH_TOKEN")
        )
        ocr_token = _validate_auth_token(
            "LOCAL_DEEPL_OCR_AUTH_TOKEN",
            _env_str("LOCAL_DEEPL_OCR_AUTH_TOKEN"),
        )
        translation_token = _validate_auth_token(
            "LOCAL_DEEPL_TRANSLATION_AUTH_TOKEN",
            _env_str("LOCAL_DEEPL_TRANSLATION_AUTH_TOKEN"),
        )
