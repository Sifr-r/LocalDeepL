...
def _parse_page_range_local(page_str: str, total_pages: int) -> list[int]:
    ...
    package init -> hybrid -> pdf, which deadlocked the ``import local_deepl
    .core.pdf`` entry point. Mirrors the canonical implementation in
    :func:`local_deepl.core.workflows.utils.parse_page_range`.
    """
    pages: set[int] = set()
    try:
...
def convert(
    ...
    expect ``from local_deepl.core.pdf.rasterizer import convert``.
