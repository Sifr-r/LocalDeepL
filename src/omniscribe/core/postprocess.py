...
class DictionaryPostProcessor:
    ...
    def _get_resource_paths(self) -> tuple[Any, str, str, str, str, str]:
        ...
        if not self._custom_resources_dir:
            packaged_dict = resources.files("omniscribe").joinpath(
    ...
    async def _try_load_custom_dict(self, dict_path: str) -> bool:
        try:
            ...
            logger.warning("Failed to load custom dictionary %s: %s", dict_path, e)
        return False
    ...
    def _replace_word(self, match: re.Match[str]) -> str:
        ...
        if not cleaned.isalpha() or spell is None:
            return word
    ...
    def correct_text(self, text: str) -> str:
        ...
        return re.sub(r"[^\W\d_]+", self._replace_word, text)
