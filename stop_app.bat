...
powershell -NoProfile -Command "$pats = 'omniscribe.server:app', 'omniscribe-server', 'omniscribe.server:main', 'celery -A omniscribe.api.tasks', 'src.omniscribe.server:app', 'celery -A src.omniscribe.api.celery_app'; Get-CimInstance Win32_Process | Where-Object { $cl = $_.CommandLine; $pats | Where-Object { $cl -like ('*' + $_ + '*') } } | ForEach-Object { Write-Host ('Hard-killing PID ' + $_.ProcessId + ' (' + $_.Name + ')'); $_ | Invoke-CimMethod -MethodName Terminate | Out-Null }"
